(() => {
  ...
  function saveCurrentTrip(){if(!window.GPSSpeedDB)throw new Error('Banco de dados não carregado');const state=getTrackingState();if(!state||state.status!=='finished')throw new Error('Finalize o percurso antes de salvar');const saved=window.GPSSpeedDB.saveTrip(buildTripRecord(state,getSpeedStats()));window.dispatchEvent(new CustomEvent('gpsspeed:trip-saved',{detail:{trip:saved}}));return saved;}
  ...
  window.GPSSpeedTrips={buildTripRecord,saveCurrentTrip,getTrips,getTrip,deleteTrip,calculateMovementTimes,buildKilometerSplits,enrichTrip,tripToGpx,buildBackup,restoreBackup,getStatistics};
})();